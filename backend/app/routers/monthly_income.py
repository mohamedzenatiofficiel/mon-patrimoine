...
router = APIRouter(prefix="/api/monthly-income", tags=["monthly-income"])
...
def upsert_monthly_income(data: MonthlyIncomeCreate, db: Session = Depends(get_db)):
...
def delete_monthly_income(income_id: int, db: Session = Depends(get_db)):
